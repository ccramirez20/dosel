...
/**
 * El cambio de contenido se anima con CSS, no con AnimatePresence: su camino de SSR
 * (PopChild) no emite los estilos que el cliente sí aplica, y React reportaba desajuste de
 * hidratación en cada carga. Con `key` basta — React remonta el nodo al cambiar la
 * selección y la animación vuelve a correr, sin estilos inline que puedan discrepar.
 */
export default function CafeInfoPanel({
  ...
  return (
    ...
    >
      <div key={key} className="panel-swap">
        {cafe ? (
          <>
            ...
            {cafe.recommendedMethods.length > 0 && (
              <p className="mt-4 font-sans text-sm text-ink/70">
                Va mejor en{" "}
                {cafe.recommendedMethods.map((id) => methodNames[id] ?? id).join(" o ")}.
              </p>
            )}
            <p className="mt-4">
              <Link
